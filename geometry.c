...
float SideLength(int a, int b, int c, int d) {
  float l = sqrt((c - a) * (c - a) + (d - b) * (d - b));
  return l;
}

void cheking(int* x1, int* y1, int* x2, int* y2, int* x3, int* y3) {
...
int main() {
  int x1, y1, r;
  printf("Введите начальную координату круга и его радиус x y r:\n");
  scanf("%d %d %d", &x1, &y1, &r);
  ...
  cheking(&xt1, &yt1, &xt2, &yt2, &xt3, &yt3);
